...
final filtreStockProvider = StateProvider<FiltreStock>((ref) =>
FiltreStock.tous);
final searchStockProvider = StateProvider<String>((ref) => '');

final medicamentsProvider = FutureProvider.autoDispose<List<Map<String, dynamic>>>((ref) async {
...
class StockScreen extends ConsumerWidget {
        ...
        children: [
            ...
            child: Column(
              children: [
                PfSearchBar(
                  ...
                  onChanged: (v) => ref.read(searchStockProvider.notifier).state = v,
                ),
          ...
          Expanded(
            child: medsAsync.when(
              ...
              error: (e, _) => Center(child: Text('Erreur: $e', style: AppTextStyles.body)),
              data: (meds) {
            ...
            child: FilterChip(
              label: Text(
                ...
                style: AppTextStyles.caption.copyWith(
                  color: selected ? Colors.white : (f.$3 ?? AppColors.textSecondary),
                  fontWeight: selected ? FontWeight.w600 : FontWeight.w500,
                ),
              ),
              selected: selected,
              onSelected: (_) => ref.read(filtreStockProvider.notifier).state = f.$1,
              backgroundColor: f.$3 != null ? f.$3!.withOpacity(0.08) : AppColors.card,
              selectedColor: f.$3 ?? AppColors.primary,
              checkmarkColor: Colors.white,
              side: BorderSide(color: f.$3 != null ? f.$3!.withOpacity(0.3) : AppColors.border),
  ...
  List<Map<String, dynamic>> _filtrerMeds(List<Map<String, dynamic>> meds,
      FiltreStock filtre) {
    final now = DateTime.now();
    final dans30j = now.add(const Duration(days: 30));

    return meds.where((m) {
      final totalBase = (m['cartons'] as int) *
          (m['cartons_par_boite'] as int) *
          (m['boites_par_plaquette'] as int) *
          (m['plaquettes_par_comprime'] as int) +
          (m['boites'] as int) * (m['boites_par_plaquette'] as int) *
              (m['plaquettes_par_comprime'] as int) +
          (m['plaquettes'] as int) * (m['plaquettes_par_comprime'] as int) +
          (m['comprimes'] as int) + (m['flacons'] as int);
      final totalBoites = (m['cartons'] as int) *
          (m['cartons_par_boite'] as int) + (m['boites'] as int);
      ...
      switch (filtre) {
        ...
        case FiltreStock.expireBientot:
          return dateExp != null && dateExp.isAfter(now) &&
  ...
  Widget _buildEmpty(FiltreStock filtre) {
    final msgs = {
      FiltreStock
          .tous: ('Aucun médicament', 'Ajoutez votre premier médicament en cliquant sur le bouton +'),
      FiltreStock
          .faible: ('Stock suffisant', 'Aucun médicament n\'est sous le seuil d\'alerte'),
      FiltreStock
          .rupture: ('Pas de rupture', 'Tous vos médicaments sont en stock'),
      FiltreStock
          .expires: ('Aucun produit expiré', 'Tous vos médicaments sont dans leur période de validité'),
      FiltreStock
          .expireBientot: ('Aucune expiration proche', 'Aucun médicament n\'expire dans les 30 prochains jours'),
    };
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              child: const Icon(Icons.medication_outlined, size: 40, color: AppColors.primary),
            ),
            const SizedBox(height: AppSpacing.md),
            Text(titre, style: AppTextStyles.h4, textAlign: TextAlign.center),
            const SizedBox(height: AppSpacing.sm),
            Text(msg, style: AppTextStyles.body.copyWith(color: AppColors.textSecondary), textAlign: TextAlign.center),
          ],
        ),
      ),
    );
...
class _MedicamentCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final totalBase = (med['cartons'] as int) *
        ...
        (med['boites'] as int) * (med['boites_par_plaquette'] as int) *
            (med['plaquettes_par_comprime'] as int) +
        (med['plaquettes'] as int) * (med['plaquettes_par_comprime'] as int) +
        (med['comprimes'] as int) + (med['flacons'] as int);
    final totalBoites = (med['cartons'] as int) *
        (med['cartons_par_boite'] as int) + (med['boites'] as int);
    ...
    bool expireBientot = dateExp != null && !expire && dateExp
        .difference(now)
        .inDays <= 30;
        ...
        child: Row(
          children: [
            ...
            Container(
              width: 4,
              height: 56,
              decoration: BoxDecoration(color: indicateur, borderRadius: AppBorderRadius.sm),
            ),
            ...
            Container(
              ...
              child: med['image_url'] != null
                  ? ClipRRect(
                borderRadius: AppBorderRadius.md,
                child: Image.network(med['image_url'], fit: BoxFit.cover,
                    errorBuilder: (_, __, ___) => const Icon(Icons.medication_outlined, color: AppColors.primary)),
              )
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      _stockChip('${med['cartons']}c ${med['boites']}b ${med['plaquettes']}p'),
                      const SizedBox(width: AppSpacing.sm),
                      if (expire) _tagChip('Expiré', AppColors.danger)
                      else if (expireBientot) _tagChip('${dateExp!.difference(now).inDays}j', AppColors.warning),
                    ],
                  ),
                ],
              ),
